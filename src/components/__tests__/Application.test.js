...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Enter the name "Leopold Silvers" into the input with the placeholder "Enter Student Name".
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    // 3. Click on the "Add" button to book an appointment.
    const appointments = getAllByTestId(container, "appointment");
    const appointment = appointments[0];

    fireEvent.click(getByAltText(appointment, "Add"));

    // 4. Enter the name "Leopold Silvers" inot the input with the placeholder "Enter Student Name".
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Leopold Silvers" },
    });
    ...
    // 7. Check that the element with the text "Saving" is dispalyed.
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
